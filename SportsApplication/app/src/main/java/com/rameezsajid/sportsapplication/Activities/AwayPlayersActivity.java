...
import com.rameezsajid.sportsapplication.Adapters.AwayPlayersAdapter;
...
public class AwayPlayersActivity extends AppCompatActivity implements AwayPlayersAdapter.OnItemClickListener {

    private RecyclerView mRecyclerView;
    private AwayPlayersAdapter mExampleAdapter;
    private ArrayList<AwayPlayersItem> mExampleList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Away Players Stats");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        relativeLayout = findViewById(R.id.relative_layout_1);

        mRecyclerView = findViewById(R.id.recycler_view_players_away);
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            JSONArray jsonArray = (JSONArray) jsonObjectAwayTeam.get("players");

                            String imageUrl = jsonObjectAwayTeam.getString("imageUrl");

                            Picasso.with(AwayPlayersActivity.this).load(imageUrl).fit().centerInside().into(imageView);

                            String name = jsonObjectAwayTeam.getString("name");
                            ...
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                mExampleList.add(new AwayPlayersItem(firstName, lastName, shirt, position, captain, goalsConceded, goalsScored, assists, shotsOnGoal, mins, saves, throwIns, goalKicks));
                            }

                            mExampleAdapter = new AwayPlayersAdapter(AwayPlayersActivity.this, mExampleList);
                            mRecyclerView.setAdapter(mExampleAdapter);
                            mExampleAdapter.setOnItemClickListner(AwayPlayersActivity.this);
    ...
    public void onItemClick(int position) {
        AwayPlayersItem clickedItem = mExampleList.get(position);
